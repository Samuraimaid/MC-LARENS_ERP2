...
import { API_BASE as API, setStoredSessionToken, setStoredUser } import React, { useState, useEffect, useCallback, useRef, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import { useAuth } from "../context/AuthContext";
import { useTheme } from "../context/ThemeContext";
...
export function LoginPage() {
  ...
  const { resolvedMode, toggleMode, setMode, setSkin, watermarkOpacity, liquidGlass } = useTheme();
  ...
  return (
    <div 
      className={cn("erp-login-root min-h-screen relative flex items-center justify-center overflow-hidden select-none bg-black font-microgramma safe-area-top safe-area-bottom pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)]", liquidGlass && "erp-liquid-shell")}
    ...
    >
      ...
      <div 
        className="absolute top-6 left-6 z-30 pointer-events-none font-microgramma will-change-transform"
        style={{ 
      ...
      {activeTool === "calculator" && (
        <div 
          className="erp-login-tool-panel absolute top-20 right-6 z-40 w-[340px] rounded-2xl border border-white/20 bg-black/85 p-4 text-white shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 font-microgramma"
      ...
      {showLoginInfo && (
        <div 
          className="erp-login-tool-panel absolute top-20 right-6 z-40 w-full max-w-sm rounded-2xl border border-white/20 bg-black/85 p-4 text-xs text-white shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 font-microgramma"
          onClick={(e) => e.stopPropagation()}
      ...
      {!isPinpadVisible && (
        <button
          ...
          className={`erp-login-fab fixed bottom-16 sm:bottom-10 left-1/2 -translate-x-1/2 z-30 inline-flex items-center justify-center gap-2.5 px-6 py-3 rounded-full bg-black/65 hover:bg-black/90 border border-white/25 backdrop-blur-xl text-white shadow-[0_8px_32px_rgba(0,0,0,0.85)] text-xs sm:text-sm font-bold tracking-wider cursor-pointer font-microgramma transition-all duration-700 max-w-[90vw] whitespace-nowrap select-none ${
            isWakeButtonVisible
      ...
      <div
        className={`erp-login-pin-panel relative z-20 w-full max-w-[340px] sm:max-w-sm mx-auto p-5 sm:p-7 rounded-3xl backdrop-blur-2xl bg-black/50 dark:bg-black/65 border border-white/20 text-white shadow-2xl transition-all duration-500 transform font-microgramma ${
